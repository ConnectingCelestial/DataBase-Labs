...
public class BTreeFile implements DbFile {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        //仅page有漏脏风险，理论上所有页都从getPage()方法获得就不需要特意放进dirtyPages
        ...
        //父结点加条目
        Field fd = page.reverseIterator().next().getField(page.keyField);
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), fd);
        newPage.setParentId(parent.getId());
        parent.insertEntry(new BTreeEntry(fd, page.getId(), newPage.getId()));

        //修改叶间指针
        if (page.getRightSiblingId() != null) {
            newPage.setRightSiblingId(page.getRightSiblingId());
            BTreeLeafPage rightLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            rightLeafPage.setLeftSiblingId(newPage.getId());
        }
        page.setRightSiblingId(newPage.getId());
        newPage.setLeftSiblingId(page.getId());

        if (field.compare(Op.LESS_THAN_OR_EQ, fd))
            return page;
        else return newPage;
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if (page.getNumEntries() != page.getMaxEntries())
            throw new DbException("can't split");
        ...
        int n = Math.ceilDiv(page.getNumEntries(), 2) - 1; //中间靠右的条目作为父节点条目
        for (int i = 0; i < n; i++) {
            BTreeEntry entry = it.next();
            page.deleteKeyAndRightChild(entry);
            BTreePage p = (BTreePage) getPage(tid, dirtypages, entry.getRightChild(), Permissions.READ_WRITE);
            p.setParentId(newPage.getId());
            newPage.insertEntry(entry);
        }
        BTreePage p = (BTreePage) getPage(tid, dirtypages, newPage.iterator().next().getLeftChild(), Permissions.READ_WRITE);
        p.setParentId(newPage.getId());

        //设置父节点条目
        BTreeEntry parentEntry = page.reverseIterator().next();
        page.deleteKeyAndRightChild(parentEntry);
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), parentEntry.getKey());
        parent.insertEntry(new BTreeEntry(parentEntry.getKey(), page.getId(), newPage.getId()));
        newPage.setParentId(parent.getId());

        if (field.compare(Op.LESS_THAN_OR_EQ, parentEntry.getKey()))
            return page;
        else return newPage;
    }
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        if (page.getNumTuples() + sibling.getNumTuples() < (page.getMaxTuples() + sibling.getMaxTuples()) / 2)
            throw new DbException("can't steal enough tuples");
        int n = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        if (isRightSibling) {
            ...
            parent.insertEntry(new BTreeEntry(sibling.iterator().next().getField(keyField), page.getId(), sibling.getId()));
        } else {
            ...
            parent.deleteKeyAndRightChild(entry);
            parent.insertEntry(new BTreeEntry(sibling.reverseIterator().next().getField(keyField), sibling.getId(), page.getId()));
        }
    }
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // some code goes here
        int n = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> it = leftSibling.reverseIterator();
        BTreePageId prevId = page.iterator().next().getLeftChild();
        ...
        for (int i = 0; i < n; i++) {
            ...
            page.insertEntry(new BTreeEntry(parentEntry.getKey(), p.getId(), prevId));
            prevId = p.getId();
            parentEntry.setKey(entry.getKey());
        }
        parent.insertEntry(new BTreeEntry(parentEntry.getKey(), leftSibling.getId(), page.getId()));
    }
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int n = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> it = rightSibling.iterator();
        BTreePageId prevId = page.reverseIterator().next().getRightChild();
        parent.deleteKeyAndRightChild(parentEntry);
        for (int i = 0; i < n; i++) {
            BTreeEntry entry = it.next();
            rightSibling.deleteKeyAndLeftChild(entry);
            BTreePage p = (BTreePage) getPage(tid, dirtypages, entry.getLeftChild(), Permissions.READ_WRITE);
            p.setParentId(page.getId());
            page.insertEntry(new BTreeEntry(parentEntry.getKey(), prevId, p.getId()));
            prevId = p.getId();
            parentEntry.setKey(entry.getKey());
        }
        parent.insertEntry(new BTreeEntry(parentEntry.getKey(), page.getId(), rightSibling.getId()));
    }
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if (leftPage.getNumTuples() + rightPage.getNumTuples() >= leftPage.getMaxTuples() + rightPage.getMaxTuples())
            throw new DbException("beyond capacity");
        ...
        if (rightPage.getRightSiblingId() == null)
            ...
            BTreeLeafPage rrp = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_ONLY);
            leftPage.setRightSiblingId(rrp.getId());
            rrp.setLeftSiblingId(leftPage.getId());
        }
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // Delete the entry in the parent corresponding to the two pages that are merging -
        // deleteParentEntry() will be useful here
        if (leftPage.getNumEntries() + rightPage.getNumEntries() >= leftPage.getMaxEntries() + rightPage.getMaxEntries())
            throw new DbException("beyond capacity");

        //父节点条目下移至左页
        deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
        BTreePageId firstPid = rightPage.iterator().next().getLeftChild();
        BTreePage p = (BTreePage) getPage(tid, dirtypages, firstPid, Permissions.READ_WRITE);
        p.setParentId(leftPage.getId());
        leftPage.insertEntry(new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), firstPid));

        //移动条目
        Iterator<BTreeEntry> it = rightPage.iterator();
        int n = rightPage.getNumEntries();
        for (int i = 0; i < n; i++) {
            BTreeEntry entry = it.next();
            rightPage.deleteKeyAndLeftChild(entry);
            //将移动的页标记为脏（指向父节点的指针改变）
            p = (BTreePage) getPage(tid, dirtypages, entry.getRightChild(), Permissions.READ_WRITE);
            p.setParentId(leftPage.getId());
