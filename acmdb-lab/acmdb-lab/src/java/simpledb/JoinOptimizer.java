...
public class JoinOptimizer {
        ...
        switch (joinOp) {
            ...
            case NOT_EQUALS:
                if (t1pkey)
                    ...
                    else card = card1 * card2 - card2;
                else if (t2pkey)
                    card = card1 * card2 - card1;
                else card = card1 * card2 - Math.max(card, card2);
                break;
            default:
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int joinNodes = joins.size();
        CostCard cc = new CostCard();
        PlanCache pc = new PlanCache();
        for (int i = 1; i <= joinNodes; i++) {
            Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> set : subsets) {
                double curBestCost = Double.MAX_VALUE;
                cc = new CostCard();
                for (LogicalJoinNode logicalJoinNode : set) {
                    CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, logicalJoinNode, set, curBestCost, pc);
                    if (costCard != null) {
                        curBestCost = costCard.cost;
                        cc = costCard;
                    }
                }
        ...
        if (explain) printJoins(cc.plan, pc, stats, filterSelectivities);
        return cc.plan;
    }
